...
class H2H(var match: Match): Fragment() {
    ...
    override fun onStart() {
        ...
        mFirebaseAnalytics?.logEvent("livescore_match_detail_h2h", null)
    ...
    private fun setupRecylerview() {
        ...
        client.newCall(request).enqueue(object: Callback {
            ...
            override fun onResponse(call: Call?, response: Response?) {
                ...
                val h2hFeed = gson.fromJson(body, H2HFeed::class.java)


                val goalScorer = ArrayList<GoalScorer>()
                var cardList = ArrayList<Cards>()

                h2hFeed.firstTeam_VS_secondTeam.forEach {
                ...
                if (activity != null) {
                    activity!!.runOnUiThread {
                        if (recylerView != null) {
                            recylerView.adapter = H2HAdapter(matchList, "")
    ...
    private fun showEmptyMessage() {
        textView_message.text = "İki takım arasında oynanan maçlar için sistemde kayıtlı bir veri bulunmamaktadır."
