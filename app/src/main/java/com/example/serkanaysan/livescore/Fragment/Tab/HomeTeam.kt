...
class HomeTeam(var match: Match): Fragment() {
    ...
    override fun onStart() {
        ...
        mFirebaseAnalytics?.logEvent("livescore_match_detail_hometeam", null)
    ...
    private fun setupRecylerview() {
        ...
        client.newCall(request).enqueue(object: Callback {
            ...
            override fun onResponse(call: Call?, response: Response?) {
                ...
                val hometeamFeed = gson.fromJson(body, HomeTeamFeed::class.java)


                val goalScorer = ArrayList<GoalScorer>()
                var cardList = ArrayList<Cards>()

                hometeamFeed.firstTeam_lastResults.forEach {
                ...
                if (activity != null) {
                    activity!!.runOnUiThread {
                        if (recylerView != null) {
                            recylerView.adapter = H2HAdapter(matchList, match.match_hometeam_name)
    ...
    private fun showEmptyMessage() {
        textView_message.text = "Ev Sahibi takımı için sistemde kayıtlı bir maç verisi bulunmamaktadır."
